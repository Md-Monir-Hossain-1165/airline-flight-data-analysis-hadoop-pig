...
valid = FILTER flights BY year IS NOT NULL AND departure_delay IS NOT NULL;
grouped = GROUP valid BY airline;
result = FOREACH grouped GENERATE group AS airline_code, AVG(valid.departure_delay) AS average_departure_delay_minutes;
ordered = ORDER result BY average_departure_delay_minutes DESC;
STORE ordered INTO 'E:/BDAL/Final_Assignment/Flight_Data_Analysis/PigAnalysis/output/05_average_departure_delay_by_airline' USING PigStorage('|');
